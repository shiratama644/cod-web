...
- PH2-A で `SimProfile` contract + `TYPE_SPECS`、PH2-B で `profile-fps` の `createFpsSimProfile()`、PH2-C で gameserver runtime profile 注入、PH2-D で web `GameClient` / `ClientPrediction` profile 注入を追加済み。`engine-core` は L1 なので `@cod/profile-fps` / `@cod/profile-voxel` import と `if (type === 'fps' | 'voxel')` を入れない。次は PH2-E: same input + determinism + docs 整理。
- PH2-C で `Room` / `Simulation` / `SnapshotBroadcaster` は既存 constructor 互換を維持しつつ profile-like seam を受けるようになり、`apps/gameserver/src/runtime.ts` の `createDefaultServerRuntime()` が `createFpsSimProfile()` から組み立てる。PH2-D で `apps/web/src/game/net/GameClient.ts` は `ClientSimProfile`、`prediction.ts` は `ClientPredictionProfile` を受けるようになり、default fps は `createFpsSimProfile()` 経由で維持した。
