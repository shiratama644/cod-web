...
>
> - **進捗の正本は `docs/ROADMAP.md`**。計画書は個別タスクの詳細 (目的・範囲・条件) を担う。
> - **このディレクトリの規則・目次は `docs/planning/README.md`**。計画書の置き方・命名・役割分担はそちらを参照。
> - 各フェーズの計画書は本テンプレートの §1〜§9 を必須セクションとし、
>   §10〜§12 (設計詳細・Gotchas・実績) を必要に応じて増減する。
>   (§10〜§12 は旧計画書の良い点 — 詳細仕様・はまりどころ記録・実測値 — を継承する場所)
> - 新規タスクを計画する場合は必ず `docs/ROADMAP.md` に行を追加 (ID 採番) してから本テンプレートで詳細化する。
...
> 対応タスク ID: `PXX-Y` (docs/ROADMAP.md)
