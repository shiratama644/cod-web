...
- 関連仕様（[`../CONFIG.md`](../CONFIG.md) / [`../../AGENTS.md`](../../AGENTS.md) §6 / [`.agent/skills/`](../../.agent/skills)）を読む
...
- Three.js / R3F のシーンとゲームループの骨架を置き、CONFIG.md の黄金ルール（**ゲームループと React レンダリングの分離**・**ゼロアロケーション**）を初期コードの段階で体現して、後続フェーズがこのパターンを踏襲できるようにする。
...
- 推測でライブラリのバージョンや API を決めず、CONFIG.md に記載のない主要ライブラリを導入する場合はユーザーに確認する（§7 停止条件）。
