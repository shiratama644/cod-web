...
| P1-D | 60Hz 権威シミュレーション＋入力受信（アキュムレータ固定 1/60・最新入力消費・shared stepPlayer 権威実行・移動検証） | ローカル検証済み | 100% | P1-B,P1-C | 入力駆動で位置が進む統合テスト | `server/sim/Simulation.ts`（固定 1/60 ステップ・アキュムレータ・MAX_STEPS クランプ・プレイヤーごと最新入力1つ消費・seq 巻き戻り無視・入力なしはアイドルで重力）＋`server/physics/world.ts`（平面＋障害物の BVH 構築・ヘッドレス）。`index.ts` の WS message でバイナリ入力を decode して sim へ、60Hz ループで `sim.update(performance.now())`。テスト **4 件**（落下着地・移動入力で前進・古いseq無視・アキュムレータ分解）green。全36 test・typecheck2構成・lint39files・build PASS |
