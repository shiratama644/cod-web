...
| PH1.5-D | Quality gate docs / CI 提案整理 | ローカル検証済み | 100% | PH1.5-C | coverage threshold ratchet 方針、E2E 実行手順、次 Phase 2 handoff が docs に反映される | 本コミット / `docs/ops/README.md`, `docs/ops/quality-gates.md`, `docs/ops/github-actions-proposal.yml` 追加 / 当時は `.github/workflows/` への直接書き込みが禁止で配置提案のみだったが、2026-09-19 に許可され本番配置へ / Playwright CI・webServer、Bun `bun ci`、GitHub Actions workflow syntax、setup-bun 公式情報を再確認 / Phase 2 handoff 更新 / typecheck・lint・unit・coverage・build・E2E discovery・link check・git diff --check pass |
...
| ID | タスク | 状態 | 進捗 | 依存 | 完了条件 | 証拠 |
|---|---|---|---:|---|---|---|
| CI-1 | GitHub Actions を `.github/workflows/` に本番配置（2026-09-19 許可） | 完了 | 100% | PH1.5-D | `docs/ops/github-actions-proposal.yml` と `.github/workflows/quality-gates.yml` が同期され、CI で typecheck/lint/determinism/unit/coverage/build/E2E discovery が実行される | 本コミット / `.github/workflows/quality-gates.yml` 追加 / 旧「書き込み不可」表記は AGENTS.md §6.3 更新で解除 |
