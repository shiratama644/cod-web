...
| **4** | ハブ + Sandbox モーダル + 投票 + マッチメイカー骨組み | PH4-F ローカル検証済み（Official FPS 1ゲーム複数モード + Voxel 1モード永続、Sandbox公式拡張+UGC、Header Official切替、Sidebar Sandbox、親ジャンル+サブタグフィルタ、詳細ページ Play Now/Room Selection、投票入口） |
...
| PH4-A | GameModeDefinition 親ジャンル+サブタグ+subModes+表示/統計拡張 | ローカル検証済み | 100% | PLAT-4 | `gamemode-api`にparentGenre/genres(サブタグ)/tags/display/stats/subModes/currentSubMode/category optional追加、ffa拡張 Official FPSのFFAサブモード、後方互換維持、tests追加 | 本コミット / `packages/gamemode-api/src/types.ts` ParentGenre/SubTag/Tag/GameCategory/PlatformCategory/ParentGenreFilter/SubTagFilter/SandboxSortKey/SandboxCard/SandboxQuery/VoteOption/VoteSession/VoteEvent/VoxelSurvivalSpec/GameModeDisplay/GameModeStats追加、RoomStateにvoting追加、GameModeDefinitionにparentGenre/genres/tags/display/stats/subModes/currentSubMode/category optional追加 / `defineGameMode.ts` parentGenre/genres/tags/subModes/currentSubMode/category/display/stats軽量検証、空配列許容、後方互換維持 / `gamemodes/fps/official/ffa/index.ts` parentGenre fps, genres [ffa], tags [official,pvp,fps], subModes [ffa,tdm,dom], currentSubMode ffa, category Official, display title/creator/thumbnail, stats totalPlays/activePlayers/detailViews追加 / `_tests_/packages/gamemode-api/src/defineGameMode.test.ts` 10→19 tests (+9) parentGenre/genres/tags/subModes/category/display/stats/empty array/Sandbox official extension/voxel survival/後方互換 / `_tests_/gamemodes/fps/official/ffa.test.ts` 改訂版フィールド検証追加 / typecheck pass / lint 0 warnings / test:unit 37 files 264 tests pass (+9) / coverage 93.45%/87.14%/86.8%/94.9% thresholds 85 pass / build pass / determinism pass |
| PH4-B | ハブUI Header Official FPS/Voxelタブ切替 | ローカル検証済み | 100% | PH4-A | Headerコンポーネント [FPS][Voxel] Official切替、activeTab state、App統合、tests | `8a162dc` / Header.tsx Official FPS 1ゲーム複数モード / Voxel Survival永続タブ、activeTab store、controlled props、data-testid / gameStore activeTab+setActiveTab / App Header統合 app-game-area / index.css hub-headerスタイル / Header.test.tsx 5 tests / typecheck/lint/unit 38 files 269 tests/build/E2E 11 pass |
| PH4-C | Left Sidebar Krunker風 + Sandboxボタン (公式拡張+UGC) | ローカル検証済み | 100% | PH4-B | LeftSidebarコンポーネント、Sandboxボタンでモーダルopen 公式拡張+UGC、GameStore sandboxOpen、App統合、tests | `9e5fb13` / LeftSidebar Play/Sandbox/Settings/Shop、Sandboxボタン 公式拡張+UGC、store sandboxOpen / App app-body flex / index.css left-sidebarスタイル / App2.test Header/Sidebarモック / LeftSidebar.test 5 tests / typecheck/lint/unit 39 files 274 tests/build/E2E 11 pass |
| PH4-D | Sandboxモーダル 親ジャンル+サブタグフィルタ+ソート (mock 公式拡張+UGC) | ローカル検証済み | 100% | PH4-C | sandbox.ts parentGenre+subTag filter/sort、matchmaker-mock.ts mockGameModes 公式拡張+UGC、SandboxModal カード thumbnail/title/creator/plays/desc creator Official含む、親ジャンル FPS/Voxel + サブタグ Bedwars/Zombie/Athletic、ソート plays/active/views、GameStore filter/sort、tests | `461fb2a` / sandbox.ts MOCK_CARDS 6件 公式拡張+UGC、filterByParentGenre/SubTag/Search/sortByKey/applySandboxFilters / matchmaker-mock.ts mockGameModes/mockRooms/fetchGameModes/fetchGameList/seekGame / gameStore sandboxParentGenre/SubTag/Sort/Search/selectedId / SandboxModal 親ジャンル+サブタグフィルタ+ソート、カード Official含む / sandbox.test 7 tests / SandboxModal.test 8 tests / typecheck/lint/unit 41 files 289 tests/build pass |
| PH4-E | 詳細ページ + Play Now/Room Selectionモーダル (mock) | ローカル検証済み | 100% | PH4-D | SandboxDetailPage、RoomSelectionModal、matchmaker-mock拡張 mockRooms/fetchGameList/seekGame、Play Now auto-match mock、Room Selection manual mock 公式拡張+UGC、tests | `19ef829` / SandboxDetailPage 詳細内部 /{type}/{source}/{slug}→/sandbox/{id}、公式拡張+UGC、Play Now seekGame auto-match、Room Selection manual、close / RoomSelectionModal ルーム一覧 mockRooms/fetchGameList、Join seekGame / gameStore roomSelectionOpen追加 / App統合 / Detail 7 tests / Room 6 tests / typecheck/lint/unit 43 files 302 tests/build pass |
| PH4-F | 投票システム入口 Official FPS 1ゲーム複数モード (mock) + quality gate | ローカル検証済み | 100% | PH4-E | VoteOverlay Official FPS FFA/TDM/DOM subMode投票 Voxel投票対象外、voteSession subMode state、onRoundEnd mockで投票UI、FFA/TDM/DOM候補、多数決、typecheck/lint/unit 255+ /coverage 85%/build/E2E 11+/determinism pass、task-list/HANDOFF/quality-gates更新 | 本コミット / VoteOverlay.tsx Official FPS 1ゲーム複数モード FFA/TDM/DOM投票、Voxel投票対象外 Survival永続表示、timer/total、resultsソート、store連携 / gameStore voteSession+setVoteSession追加 / App統合 / index.css vote-overlayスタイル / VoteOverlay.test.tsx 8 tests 非表示/表示/header/Voxel対象外/timer/total/store投票/controlled/close/ソート/store連携 / typecheck pass / lint 132 files 0 warnings / test:unit 44 files 311 tests / coverage 93.51%/85.8%/88.48%/94.76% thresholds 85 pass / build pass / E2E list 11 / determinism + heavy pass / task-list/HANDOFF/quality-gates更新予定 |
