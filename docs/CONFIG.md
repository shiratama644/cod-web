...
| ライブラリ | 役割 | 備考 |
| :--- | :--- | :--- |
| **Colyseus** | （参考）権威型ゲームサーバーフレームワーク | ルーム管理・状態同期を持つが、**本プロジェクトでは廃止**（WebRTC/UDP のゲーム同期に未対応のため） |
| **geckos.io** | UDP over WebRTC | **本プロジェクト採用（ゲーム同期層）**。FPS に必要なパケットロス許容・超低遅延通信（プレイヤー座標・入力・Snapshot・射撃イベント同期） |
| **socket.io** | WebSocket通信 | **本プロジェクト採用（制御系）**。認証、ロビー、ルーム、チャット、マッチイベントなど信頼性・確実性が求められる通信 |
