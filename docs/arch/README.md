...
| **Agent 記憶** | [`../../.agent/`](../../.agent) | skills（コードベース事実の要約）/ hooks（定型手順）/ logs（実行記録） | skills は正本（arch）を要約・参照する |
...
| [networking.md](./networking.md) | ネットワーク＆リアルタイム設計の決定記録（WebTransport 主 / WebSocket フォールバック、30Hz tick、msgpackr、FX クライアント再生、サーバーランタイム構成） |
