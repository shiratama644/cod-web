...
> サーバー tick・スナップショットは **30Hz**、入力送信は **60Hz**、描画は**可変フレームレート（60〜120Hz+）**で互いに独立。高頻度パケットは**手動バイナリ固定レイアウト**（入力~12B・20人スナップショット~330B、固定小数点量子化・ゼロアロケ）、低頻度の信頼イベントは **msgpackr**。FX/アニメはアクションフラグ（`isShooting` 等）＋発射トリガーのみ送りクライアント再生。詳細は [server-authority.md](./server-authority.md) §6。
...
## 5. エンティティ・コンポーネント・システム（データ指向 / ECS）

> **パラダイム（確定）: ゲームシミュレーション本体（`shared/sim`・権威サーバー）はデータ指向（ECS スタイル）。深い OOP 継承は使わない。**
> エンティティは整数 ID、状態はフラットなコンポーネントデータ（構造体/配列）、ロジックはそれに作用する純粋関数（システム: 移動・BVH 衝突・ラグ補償・スナップショット生成）。
> 理由: ①30/60Hz 権威シムをクライアント予測とサーバーで**同一関数**で回す決定論と相性、②ホットループの**ゼロアロケ/GC レス**、③状態がフラットで**スナップショット/巻き戻し履歴**を配列でそのまま持てる、④後で TypedArray 実装へ載せ替えやすい。
> **境界層（React/R3F コンポーネント・Room・NetTransport クライアント・XState actor・WS ソケット）は通常のクラス/React でよい**（シミュレーションのホットパスではないため）。

| ライブラリ | 役割 | 備考 |
| :--- | :--- | :--- |
| **bitecs** | **shared/サーバー権威シムのコア ECS** | TypedArray ベース・GC レス・ヘッドレス（レンダラー非依存）。決定論シミュレーションに適する。数万エンティティ（弾丸・bot 等）を配列で高速処理。Phase 1 のプレイヤー同期は ECS 形のプレーンな typed データで始め、弾丸/bot が数千体規模になる段階で導入 |
| **miniplex** / **@miniplex/react** | **クライアント表現層（R3F）の ECS** | オブジェクト志向・React 連携が得意。エンティティとメッシュ/ref を結び、ライフサイクル（弾丸・ドロップ・エフェクト）を宣言的に管理。shared/bitecs の状態を R3F シーンに反映する表現側で使用 |

> **共有データ vs 表現の分離**: 権威ロジック・予測・補間の状態は bitecs/データレイアウト（shared）に置き、R3F のメッシュとの結合・トランスフォーム反映はクライアント表現層（miniplex/R3F）が担う。サーバーは React/miniplex を持たない（ヘッドレス）。
...
8. **全端末 60FPS の性能ターゲット（最重要）**
   ...
   - **描画フレームレートは可変**: 60FPS は「全端末が達成すべき下限フロア」であり上限ではない。`requestAnimationFrame` はディスプレイのリフレッシュレート（60/90/120/144Hz）で呼ばれるため、**60 で蓋をせずハイリフレッシュ端末では 120FPS で描画**する。全移動・アニメは delta time ベース（固定 16.67ms を仮定しない）。シミュレーション tick は 60Hz 固定（描画とは独立）、スナップショット送信は 30Hz（[`networking.md`](./networking.md)）
