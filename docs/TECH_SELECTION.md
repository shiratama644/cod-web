...
**視覚エフェクトの設計方針**: エフェクト専用パケットは送らない。サーバーが毎フレーム送るプレイヤー情報（位置・向き）に **1 ビットフラグ（`isShooting: true`）** を混ぜ、クライアント側の Three.js が受け取ったら銃口からマズルフラッシュ・薬莢を**自律生成**する（エフェクトは装飾のため欠落しても進行に影響しない）。
